        ...
        if (items) {
            ...
            await ImportHelpers.asyncForEach(items, async (item) => {
                try {
                    ...
                    if (mods) {
                        if (mods?.baseMods?.attributes)
                            data.data.attributes = mods.baseMods.attributes;
                        if (mods?.baseMods?.itemattachment)
                            data.data.itemattachment = mods.baseMods.itemattachment;
                        if (mods?.baseMods?.description)
                            data.data.description += `<h3>Base Mods</h3>${mods.baseMods.description}`;
                        if (mods?.addedMods?.itemmodifier)
                            data.data.itemmodifier = mods.addedMods.itemmodifier;
                    }
